...
- Fixed concealed text becoming visible through block cursors, hyperlink foreground overrides, or native emoji. Resetting a hidden blink phase now repaints unchanged cursor and blinking-text pixels.
...
- Clarified rendering and API contracts for logical cluster columns, font resolution, bounded shaping, cache lifetimes, and EDT-only `currentSearchState()` and `preferredGridSize()` access. Documented synchronous first-use native emoji initialization and the scope of warmed allocation regressions; these checks do not establish whole-frame allocation or latency guarantees.
